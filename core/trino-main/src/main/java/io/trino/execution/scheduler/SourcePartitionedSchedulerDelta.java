        ...
        implements SourceScheduler
{
    ...
    private final int splitDeltaBatchSize;
    ...
    private SourcePartitionedSchedulerDelta(
            ...
            SplitSource splitDeltaSource,
            SplitPlacementPolicy splitPlacementPolicy,
            int splitBatchSize,
            int splitDeltaBatchSize,
            ...
            BooleanSupplier anySourceTaskBlocked)
    {
        ...
        this.splitDeltaSource = requireNonNull(splitDeltaSource, "splitSource is null");
    ...
    public static StageScheduler newSourcePartitionedSchedulerAsStageSchedulerDelta(
            SqlStageExecution stage,
            PlanNodeId partitionedNode,
            SplitSource splitSource,
            SplitSource splitDeltaSource,
            ...
            BooleanSupplier anySourceTaskBlocked)
    {
        SourcePartitionedSchedulerDelta sourcePartitionedScheduler = new SourcePartitionedSchedulerDelta(
                stage,
                partitionedNode,
                splitSource,
                splitDeltaSource,
        ...
        return new StageScheduler()
        {
            ...
            {
                ScheduleResult scheduleResult = sourcePartitionedScheduler.scheduleDelta();
                sourcePartitionedScheduler.drainCompletedDeltaLifespans();
                return scheduleResult;
            }
            ...
            public void resetDelta()
            {
                ...
                for (Pair<Lifespan, ConnectorPartitionHandle> p : sourcePartitionedScheduler.lifespans){
                    ...
                    sourcePartitionedScheduler.whenFinishedOrNewLifespanAddedDelta = SettableFuture.create();
                }
            }
    ...
    public static SourceScheduler newSourcePartitionedSchedulerAsSourceScheduler(
            ...
            BooleanSupplier anySourceTaskBlocked)
    {
        return new SourcePartitionedSchedulerDelta(
                stage,
                partitionedNode,
                splitSource,
                splitDeltaSource,
    ...
    public synchronized void startLifespan(Lifespan lifespan, ConnectorPartitionHandle partitionHandle)
    {
        ...
        lifespans.add(new Pair<>(lifespan, partitionHandle));
    ...
    @Override
    public synchronized ScheduleResult scheduleDelta()
    {
        dropListenersFromWhenFinishedOrNewLifespansAddedDelta();
        ...
        for (Entry<Lifespan, ScheduleGroup> entry : scheduleGroupsDelta.entrySet()) {
            ...
            if (scheduleGroup.state == ScheduleGroupState.NO_MORE_SPLITS || scheduleGroup.state == ScheduleGroupState.DONE) {
                ...
                // try to get the next batch
                if (scheduleGroup.nextSplitBatchFuture == null) {
                    scheduleGroup.nextSplitBatchFuture = splitDeltaSource.getNextBatch(scheduleGroup.partitionHandle, lifespan, splitDeltaBatchSize - pendingSplits.size());
            ...
            if (!pendingSplits.isEmpty()) {
                ...
                if (stateDelta == State.INITIALIZED) {
                    stateDelta = State.SPLITS_ADDED;
                }
        ...
        // * `splitSource.isFinished` invocation may fail after `splitSource.close` has been invoked.
        //   If stateDelta is NO_MORE_SPLITS/FINISHED, splitSource.isFinished has previously returned true, and splitSource is closed now.
        // * Even if `splitSource.isFinished()` return true, it is not necessarily safe to tear down the split source.
        //   * If anyBlockedOnNextSplitBatch is true, it means we have not checked out the recently completed nextSplitBatch futures,
        //     which may contain recently published splits. We must not ignore those.
        //   * If any scheduleGroup is still in DISCOVERING_SPLITS stateDelta, it means it hasn't realized that there will be no more splits.
        //     Next time it invokes getNextBatch, it will realize that. However, the invocation will fail we tear down splitSource now.
        if ((stateDelta == State.NO_MORE_SPLITS || stateDelta == State.FINISHED) || (noMoreScheduleGroups && scheduleGroupsDelta.isEmpty() && splitSource.isFinished())) {
            switch (stateDelta) {
                case INITIALIZED:
                    // We have not scheduled a single split so far.
                    // But this shouldn't be possible. See usage of EmptySplit in this method.
                    throw new IllegalStateException("At least 1 split should have been scheduled for this plan node");
                case SPLITS_ADDED:
                    stateDelta = State.NO_MORE_SPLITS;
                    splitSource.close();
                    // fall through
                case NO_MORE_SPLITS:
                    stateDelta = State.FINISHED;
                    whenFinishedOrNewLifespanAddedDelta.set(null);
                    // fall through
                case FINISHED:
                    return new ScheduleResult(
                            true,
        ...
        overallBlockedFutures.add(whenFinishedOrNewLifespanAddedDelta);
        return new ScheduleResult(
                false,
                overallNewTasks.build(),
                nonCancellationPropagating(asVoid(whenAnyComplete(overallBlockedFutures))),
    ...
    private synchronized void dropListenersFromWhenFinishedOrNewLifespansAddedDelta()
    {
        // whenFinishedOrNewLifespanAddedDelta may remain in a not-done stateDelta for an extended period of time.
        // As a result, over time, it can retain a huge number of listener objects.

        // Whenever schedule is called, holding onto the previous listener is not useful anymore.
        // Therefore, we drop those listeners here by recreating the future.

        // Note: The following implementation is thread-safe because whenFinishedOrNewLifespanAddedDelta can only be completed
        // while holding the monitor of this.

        if (whenFinishedOrNewLifespanAddedDelta.isDone()) {
            return;
        }

        whenFinishedOrNewLifespanAddedDelta.cancel(true);
        whenFinishedOrNewLifespanAddedDelta = SettableFuture.create();
    }
    ...
    public synchronized List<Lifespan> drainCompletedLifespans()
    {
        ...
        // TODO, only remove lifespans once we are completely finished
        ...
        if (scheduleGroups.isEmpty() && splitSource.isFinished()) { // TODO: only transition if delta splits are finished too, but what if delta never got called
    ...
    {
        if (scheduleGroupsDelta.isEmpty()) {
            // Invoking splitSource.isFinished would fail if it was already closed, which is possible if scheduleGroupsDelta is empty.
            return ImmutableList.of();
        }

        // TODO, only remove lifespans once we are completely finished
        ImmutableList.Builder<Lifespan> result = ImmutableList.builder();
        Iterator<Entry<Lifespan, ScheduleGroup>> entryIterator = scheduleGroupsDelta.entrySet().iterator();
        while (entryIterator.hasNext()) {
            Entry<Lifespan, ScheduleGroup> entry = entryIterator.next();
            if (entry.getValue().state == ScheduleGroupState.DONE) {
                result.add(entry.getKey());
        ...
        if (scheduleGroupsDelta.isEmpty() && splitDeltaSource.isFinished()) { // TODO: only transition if delta splits are finished too, but what if delta never got called
            // Wake up blocked caller so that it will invoke schedule() right away.
            // Once schedule is invoked, state will be transitioned to FINISHED.
            whenFinishedOrNewLifespanAddedDelta.set(null);
            whenFinishedOrNewLifespanAddedDelta = SettableFuture.create();
