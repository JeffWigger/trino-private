...
public class DeltaRecordPageSource
        implements ConnectorPageSource
{
    private static final int ROWS_PER_REQUEST = 4096;
    private final DeltaRecordCursor cursor;
    private final List<Type> types;
    private final DeltaPageBuilder pageBuilder;
    private boolean closed;

    public DeltaRecordPageSource(DeltaRecordSet recordSet)
    {
        this(requireNonNull(recordSet, "recordSet is null").getColumnTypes(), recordSet.cursor());
    }

    public DeltaRecordPageSource(List<Type> types, DeltaRecordCursor cursor)
    {
    ...
    {
        if (!closed) {
            int updateBytePosition = types.size();
            for (int i = 0; i < ROWS_PER_REQUEST && !pageBuilder.isFull(); i++) {
                ...
                pageBuilder.updateBuilder.writeByte(DeltaPageBuilder.Mode.valueOf(cursor.getSlice(updateBytePosition).toStringUtf8()).ordinal());
            }
        }
