...
public class UpdatablePageBuilder
{
    ...
    private final UpdatableBlock[] blockBuilders;
     ...
     */
    public UpdatablePageBuilder(List<? extends Type> types)
    {
        this(DEFAULT_INITIAL_EXPECTED_ENTRIES, types);
    }

    public UpdatablePageBuilder(int initialExpectedEntries, List<? extends Type> types)
    {
    ...
    private UpdatablePageBuilder(int initialExpectedEntries, int maxPageBytes, List<? extends Type> types, Optional<UpdatableBlock[]> templateBlockBuilders)
    {
        ...
        blockBuilders = new UpdatableBlock[types.size()];

        if (templateBlockBuilders.isPresent()) {
            UpdatableBlock[] templates = templateBlockBuilders.get();
    ...
    public UpdatablePageBuilder newPageBuilderLike()
    {
        return new UpdatablePageBuilder(declaredPositions, pageBuilderStatus.getMaxPageSizeInBytes(), types, Optional.of(blockBuilders));
    }
    ...
    public UpdatablePage build()
    {
        if (blockBuilders.length == 0) {
            return new UpdatablePage(declaredPositions);
        }

        UpdatableBlock[] blocks = new UpdatableBlock[blockBuilders.length];
        for (int i = 0; i < blocks.length; i++) {
            blocks[i] = blockBuilders[i];
