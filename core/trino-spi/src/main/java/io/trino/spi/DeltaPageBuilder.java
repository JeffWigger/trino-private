...
public class DeltaPageBuilder
{
     ...
     */
    public DeltaPageBuilder(List<? extends Type> types)
    {
    ...
    private DeltaPageBuilder(int initialExpectedEntries, int maxPageBytes, List<? extends Type> types, Optional<BlockBuilder[]> templateBlockBuilders)
    {
        ...
        if (templateBlockBuilders.isPresent()) {
            ...
            // TODO: have no template for updateBuilder
    ...
    public void reset()
    {
        ...
        updateBuilder = updateBuilder.newBlockBuilderLike(pageBuilderStatus.createBlockBuilderStatus());
    }

    public DeltaPageBuilder newPageBuilderLike()
    {
        return new DeltaPageBuilder(declaredPositions, pageBuilderStatus.getMaxPageSizeInBytes(), types, Optional.of(blockBuilders));
    }
    ...
    public DeltaPage build()
    {
        ...
        if (updateBuilder.getPositionCount() != declaredPositions) {
            throw new IllegalStateException(format("Declared positions (%s) does not match update type block's number of entries (%s)", declaredPositions, updateBuilder.getPositionCount()));
        }
