...
public class UpdatableIntArrayBlock
        implements UpdatableBlock
{
    private static final int INSTANCE_SIZE = ClassLayout.parseClass(UpdatableIntArrayBlock.class).instanceSize();
    ...
    public UpdatableIntArrayBlock(@Nullable BlockBuilderStatus blockBuilderStatus, int expectedEntries)
    {
    ...
    public UpdatableIntArrayBlock(@Nullable BlockBuilderStatus blockBuilderStatus, int positionCount, boolean[] valueMarker, int[] values)
    {
    ...
    public UpdatableIntArrayBlock(@Nullable BlockBuilderStatus blockBuilderStatus, int positionCount, byte[] valueMarker, int[] values, int nullCounter, int deleteCounter)
    {
        if (positionCount < 0) {
    ...
    @Override
    public UpdatableIntArrayBlock writeInt(int value)
    {
    ...
    @Override
    public UpdatableIntArrayBlock updateInt(Integer value, int position, int offset)
    {
    ...
    @Override
    public UpdatableIntArrayBlock deleteInt(int position, int offset)
    {
        checkReadablePosition(position);
        if (offset != 0) {
            throw new IllegalArgumentException("offset must be zero");
    ...
    @Override
    public UpdatableIntArrayBlock closeEntry()
    {
        return this;
    }

    @Override
    public UpdatableIntArrayBlock appendNull()
    {
        if (values.length <= positionCount) {
            growCapacity();
        }
    ...
    public Block build()
    {
        ...
        if (deleteCounter == positionCount) {
            return new IntArrayBlock(0, 0, null, new int[0]);
        }
        compact();
        return new IntArrayBlock(0, values.length, mayHaveNull() ? toBoolean(valueMarker) : null, values);
    }

    @Override
    public UpdatableIntArrayBlock newBlockBuilderLike(BlockBuilderStatus blockBuilderStatus)
    {
        return new UpdatableIntArrayBlock(blockBuilderStatus, calculateBlockResetSize(positionCount));
    }
    ...
    public Block copyPositions(int[] positions, int offset, int length)
    {
        ...
        return new IntArrayBlock(0, acctualSize, Arrays.copyOfRange(newValueIsNull, 0, acctualSize), Arrays.copyOfRange(newValues, 0, acctualSize));
    }
    ...
    {
        StringBuilder sb = new StringBuilder("UpdatabeIntArrayBlock{");
    ...
    private void compact()
    {
        if (nullCounter != 0) {
            ...
            if (blockBuilderStatus != null) {
                blockBuilderStatus.addBytes(-(Byte.BYTES + Integer.BYTES) * (positionCount - c.values.length));
            }
    ...
    {
        int[] compacted = new int[positionCount - nullCounter];
    ...
    private class Core
    {
        ...
        public Core(int[] values, byte[] markers)
        {
            this.values = values;
    ...
    private class CoreBool
    {
        ...
        public CoreBool(int[] values, boolean[] markers)
        {
            this.values = values;
            this.markers = markers;
        }
